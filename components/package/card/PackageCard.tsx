...
export function PackageCard({ pkg, index }: PackageCardProps) {
  ...
  return (
    ...
    >
      ...
      {pkg.stars > 1000 && (
        <span className="absolute top-2.5 right-3 bg-linear-to-r from-[#00ADD8] to-cyan-600 dark:from-sky-700 dark:to-cyan-800 text-white font-bold text-[9px] uppercase tracking-wider px-2 py-0.5 rounded-md shadow select-none">
          High Demand
        </span>
      )}
      ...
      <div className="min-w-0 flex-1 space-y-2">
        <div className="flex flex-wrap items-center gap-2 select-none">
          ...
          {pkg.category && (
            <span className="text-[9px] uppercase tracking-wider font-bold text-[#007D9C] dark:text-sky-400 bg-sky-50 dark:bg-sky-950/30 px-2.5 py-0.5 rounded-full border border-sky-100/50 dark:border-sky-800/50 shrink-0">
              {pkg.category}
            </span>
          )}

          {pkg.license && (
            <span className="text-[9px] bg-emerald-50 dark:bg-emerald-950/20 text-emerald-800 dark:text-emerald-400 px-1.5 py-0.5 rounded border border-emerald-200 dark:border-emerald-900/30 font-mono font-bold uppercase flex items-center gap-1 shrink-0">
          ...
          {pkg.latestVersion && (
            <span className="text-[9px] bg-slate-50 dark:bg-[#161b22] text-slate-500 dark:text-[#8b949e] px-1.5 py-0.5 rounded border border-slate-200 dark:border-[#30363d] font-mono font-semibold shrink-0">
              {pkg.latestVersion}
            </span>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500 dark:text-[#8b949e] relative z-10 pt-1 select-none font-sans">
          {pkg.stars > 0 && (
            <div className="relative group/tooltip flex items-center gap-1 font-semibold text-[#007D9C] dark:text-sky-400 bg-[#00ADD8]/8 dark:bg-sky-950/40 px-2.5 py-0.5 rounded-full border border-[#00ADD8]/20 dark:border-sky-800/50 hover:bg-[#00ADD8]/15 dark:hover:bg-sky-900/40 transition-colors">
