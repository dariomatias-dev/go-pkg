    ...
    <div className="space-y-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-[#30363d] dark:bg-[#161b22]">
      <div className="flex items-center justify-between border-b border-slate-100 pb-3 dark:border-[#30363d]">
        ...
        {hasFilter && (
          <button
            onClick={onClearAll}
            className="cursor-pointer text-[11px] font-medium text-rose-600 hover:underline"
          >
            Clear All
          </button>
        )}
      ...
      <div className="space-y-2">
        <span className="block text-xs font-bold tracking-wider text-slate-500 uppercase dark:text-[#8b949e]">
          Categories
        </span>
        ...
        <div className="space-y-1">
          <button
            ...
            className={cn(
              ...
              !category
                ? "bg-sky-50 font-semibold text-[#006680] dark:bg-[#21262d] dark:text-sky-400"
          ...
          {categories.map((cat) => (
            <button
              ...
              className={cn(
                "flex w-full cursor-pointer items-center justify-between rounded-lg px-3 py-2 text-left text-xs transition-colors",
                category === cat.id
                  ? "bg-[#E0F2FE] font-semibold text-[#006680] dark:bg-[#21262d] dark:text-sky-400"
                  : "text-slate-600 hover:bg-slate-50 dark:text-[#c9d1d9] dark:hover:bg-[#21262d]",
              )}
            >
